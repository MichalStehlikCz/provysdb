...
public enum Function {
  ...
  /**
   * Type that resulting expression will have.
   * There is one special hack - in case function returns same type as one of arguments (e.g.
   * COALESCE), Object is used as return type and resultByType is set to value >= 0.
   */
  private final Class<?> result;
  private final int resultAsArgument;
  /**
   * List of types of arguments.
   * Special case are functions that have exactly three arguments and the first argument has same
   * type as result - system allows to repeat second argument of such function, resulting in nested
   * calls. This is also used for operators.
   */
